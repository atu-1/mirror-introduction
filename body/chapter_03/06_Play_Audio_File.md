...
オーディオファイルを再生する前に、再生するオーディオファイルを選択します。オーディオファイルの選択処理は ```SelectAudioFile``` で行い、[2-10節](../chapter_02/10_Control_Blender_UI_3.md) で説明したファイルブラウザを表示するための処理と基本的に同じですが、[2-10節](../chapter_02/10_Control_Blender_UI_3.md) のサンプルとは異なるところがあります。

1つ目の異なる点、ファイル選択時に呼び出される ```execute()``` メソッドの処理内容です。本節のサンプルではファイルを開いた後にオーディオファイルを再生する処理を記載しています。また、```execute()``` メソッドの処理では、[2-10節](../chapter_02/10_Control_Blender_UI_3.md) で宣言していたクラス変数 ```filename``` や ```directory``` は不要となるため、削除しています。
