...
描画関数の登録はエリア単位で行い、例えば *3Dビュー* エリアに描画関数を登録したい場合は ```bpy.types.SpaceView3D.draw_handler_add()``` 関数を使って登録します。ここでスペース情報 ```bpy.types.SpaceView3D``` は *3Dビュー* を描画対象とした場合ですが、描画先のエリアによってここの記述が変わります。
...
```bpy.types.SpaceView3D.draw_handler_add()``` 関数の引数には、描画関数をはじめとして次に示す値を指定します。
...
本節のサンプルでは、第1引数に ```RenderFigure.render``` 、第3引数に ```WINDOW``` を指定しています。第2引数には、自身のインスタンスとコンテキスト情報を渡しています。
