...
APIドキュメントを調べたり、他のアドオンを参考にしたりしても実現方法がわからない場合は、コミュニティサイトで質問することを検討しましょう。
...
本節では、Blenderが提供するAPIを調べる方法について紹介しました。ここまで読んでいただいたとおり、APIを調べる方法は１つではありません。筆者がAPIを調べる時は、まず最初に似たような処理を行っているアドオンのソースコードを参考にします。そしてソースコードの中でわからない部分は、公式のAPIドキュメントで仕様を確認したあと、*Pythonコンソール* を使って動作確認します。ここまで行っても欲しい情報が得られない場合は、コミュニティサイトで質問します。
